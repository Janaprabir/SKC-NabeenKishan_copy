...
class ReportStockDamage extends StatefulWidget {
  const ReportStockDamage({super.key});

  @override
  _ReportStockDamageState createState() => _ReportStockDamageState();
}

class _ReportStockDamageState extends State<ReportStockDamage> {
  String? _empId;
  int? godownId;
  ...
  Future<void> _fetchData() async {
    ...
    const String baseUrl =
        'https://www.nabeenkishan.net.in/newproject/api/routes/StockDamageController/get_stock_damage_report.php';

    String fromDateApi = DateFormat('yyyy-MM-dd')
        .format(DateFormat('dd/MM/yyyy').parse(_fromDateController.text));
    String toDateApi = DateFormat('yyyy-MM-dd')
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          for (var damageEntry in data) {
            String formattedDate = '';
            if (damageEntry['stock_damage_date']?.isNotEmpty ?? false) {
              try {
                DateTime date = DateTime.parse(damageEntry['stock_damage_date']);
                formattedDate = DateFormat('dd/MM/yyyy').format(date);
              } catch (e) {
                formattedDate = damageEntry['stock_damage_date'] ?? '';
              }
            }

            String billNo = damageEntry['bill_no'] ?? '';
            if (!billGroups.containsKey(billNo)) {
              billGroups[billNo] = {
                'Date': formattedDate,
                'Bill no': billNo,
                'Remarks': damageEntry['remarks'] ?? '',
                'Transactions': [],
              };
            }

            for (var transaction in damageEntry['transactions']) {
  ...
  Future<void> _generatePdf() async {
    ...
    try {
        ...
        pw.MultiPage(
          ...
          build: (pw.Context context) => [
            ...
            pw.Table(
              ...
              columnWidths: {
                0: const pw.FractionColumnWidth(0.14), // Date
                1: const pw.FractionColumnWidth(0.14), // Bill no
                2: const pw.FractionColumnWidth(0.22), // Remarks
                3: const pw.FractionColumnWidth(0.22), // Product name
                4: const pw.FractionColumnWidth(0.10), // Quantity
                5: const pw.FractionColumnWidth(0.14), // Pre Stock Level
                6: const pw.FractionColumnWidth(0.14), // New Stock Level
              },
              children: [
                pw.TableRow(
      ...
      final file = File(
          '${outputDirectory?.path}/stock_damage_report_${DateTime.now().millisecondsSinceEpoch}.pdf');
  ...
  Future<void> _generateExcel() async {
    ...
    try {
      ...
      sheet
          ...
          .value = excel.TextCellValue('Stock Damage Report');
      sheet
          .cell(excel.CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 0))
      ...
      final fileName =
          'stock_damage_report_${DateTime.now().millisecondsSinceEpoch}.xlsx';
  ...
  void _showFilterDialog() {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        ...
        const double dialogMinHeight = 350;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ...
          : Center(
              child: Container(
                constraints: const BoxConstraints(maxWidth: 800), // Set max width for the card
                child: Padding(
