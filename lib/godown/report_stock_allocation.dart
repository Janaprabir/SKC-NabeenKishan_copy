...
class ReportStockAllocation extends StatefulWidget {
  const ReportStockAllocation({super.key});

  @override
  _ReportStockAllocationState createState() => _ReportStockAllocationState();
}

class _ReportStockAllocationState extends State<ReportStockAllocation> {
  String? _empId;
  int? godownId;
  ...
  Future<void> _fetchData() async {
    ...
    const String baseUrl =
        'https://www.nabeenkishan.net.in/newproject/api/routes/StockDispatchController/get_stock_dispatch_report.php';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          // Group transactions by DCN
          Map<String, Map<String, dynamic>> dcnGroups = {};
          for (var dispatch in data) {
            String formattedDate = '';
            if (dispatch['stock_dispatch_date']?.isNotEmpty ?? false) {
              try {
                DateTime date = DateTime.parse(dispatch['stock_dispatch_date']);
                formattedDate = DateFormat('dd/MM/yyyy').format(date);
              } catch (e) {
                formattedDate = dispatch['stock_dispatch_date'] ?? '';
              }
            }

            String dcn = dispatch['dcn'] ?? '';
            if (!dcnGroups.containsKey(dcn)) {
              dcnGroups[dcn] = {
                'Employee Name':
                    '${dispatch['emp_first_name'] ?? ''} ${dispatch['emp_middle_name'] ?? ''} ${dispatch['emp_last_name'] ?? ''}',
                'Date': formattedDate,
                'DCN No': dcn,
                'Remarks': dispatch['remarks'] ?? '',
                'Address': dispatch['address'] ?? '',
                'Trip No': dispatch['trip_no'] ?? '',
            ...
            for (var transaction in dispatch['transactions']) {
              dcnGroups[dcn]?['Transactions'].add({
          ...
          for (var dcn in dcnGroups.values) {
            for (var transaction in dcn['Transactions']) {
              _records.add({
                'Employee Name': dcn['Employee Name'],
                'Date': dcn['Date'],
                'DCN No': dcn['DCN No'],
                'Remarks': dcn['Remarks'],
                'Address': dcn['Address'],
                'Trip No': dcn['Trip No'],
                'Vehicle No': dcn['Vehicle No'],
                'Driver Name': dcn['Driver Name'],
                ...
                'IsDcnHeader': dcn['Transactions'].indexOf(transaction) == 0,
              });
            }
          }
        });
  ...
  Future<void> _generatePdf() async {
    ...
    try {
        ...
        pw.MultiPage(
          ...
          header: (context) => pw.Container(
            ...
            child: pw.Column(
              ...
              children: [
                pw.Text('Stock Allocation Report',
                    style: pw.TextStyle(
                        fontSize: 24,
          ...
          build: (pw.Context context) => [
            ...
            pw.Table(
              ...
              columnWidths: {
                ...
                2: const pw.FractionColumnWidth(0.10), // DCN No
                3: const pw.FractionColumnWidth(0.10), // Remarks
                4: const pw.FractionColumnWidth(0.10), // Address
                5: const pw.FractionColumnWidth(0.08), // Trip No
                6: const pw.FractionColumnWidth(0.08), // Vehicle No
                7: const pw.FractionColumnWidth(0.08), // Driver Name
                8: const pw.FractionColumnWidth(0.15), // Product name
                9: const pw.FractionColumnWidth(0.08), // Quantity
                10: const pw.FractionColumnWidth(0.08), // Pre Stock Level
                11: const pw.FractionColumnWidth(0.08), // New Stock Level
              },
              children: [
                pw.TableRow(
                      ...
                      children: _columns.map((column) {
                        final alignment = (column == 'Quantity' ||
                            ...
                            : (column == 'DCN No' ||
                                ...
                                ? pw.TextAlign.center
                                : pw.TextAlign.left;
                        // Show dispatch-level fields only on the first transaction of the DCN
      ...
      final file = File(
          '${outputDirectory?.path}/stock_allocation_report_${DateTime.now().millisecondsSinceEpoch}.pdf');
  ...
  Future<void> _generateExcel() async {
    ...
    try {
      ...
      sheet
          ...
          .value = excel.TextCellValue('Stock Allocation Report');
      sheet
          .cell(excel.CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 0))
      ...
      for (int row = 0; row < _records.length; row++) {
        ...
        for (int col = 0; col < _columns.length; col++) {
          ...
          final value = (_columns[col] == 'Employee Name' ||
                  ...
                  _columns[col] == 'Vehicle No' ||
                  _columns[col] == 'Driver Name') &&
              !record['IsDcnHeader']
              ? ''
              : record[_columns[col]] ?? '';
      ...
      final fileName =
          'stock_allocation_report_${DateTime.now().millisecondsSinceEpoch}.xlsx';
