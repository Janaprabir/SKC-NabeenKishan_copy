...
class ReportStockIn extends StatefulWidget {
  const ReportStockIn({super.key});

  @override
  _ReportStockInState createState() => _ReportStockInState();
}

class _ReportStockInState extends State<ReportStockIn> {
  String? _empId;
  int? godownId;
  ...
  Future<void> _fetchData() async {
    ...
    const String baseUrl =
        'https://www.nabeenkishan.net.in/newproject/api/routes/StockInController/get_stock_in_report.php';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          // Group transactions by bill number
          Map<String, Map<String, dynamic>> billGroups = {};
          for (var stockIn in data) {
            String formattedDate = '';
            if (stockIn['stock_in_date']?.isNotEmpty ?? false) {
              try {
                DateTime date = DateTime.parse(stockIn['stock_in_date']);
                formattedDate = DateFormat('dd/MM/yyyy').format(date);
              } catch (e) {
                formattedDate = stockIn['stock_in_date'] ?? '';
              }
            }

            String billNo = stockIn['bill_no'] ?? '';
            ...
            for (var transaction in stockIn['transactions']) {
              billGroups[billNo]?['Transactions'].add({
                'Product': transaction['product_name'] ?? '',
                'Quantity': transaction['quantity']?.toString() ?? '',
                'Pre Stock': transaction['pre_stock_level']?.toString() ?? '',
                'New Stock': transaction['new_stock_level']?.toString() ?? '',
          ...
          for (var bill in billGroups.values) {
            for (var transaction in bill['Transactions']) {
              _records.add({
                ...
                'Pre Stock': transaction['Pre Stock'],
                'New Stock': transaction['New Stock'],
  ...
  Future<void> _generatePdf() async {
    ...
    try {
          ...
          build: (pw.Context context) => [
            ...
            pw.Table(
              ...
              columnWidths: {
                ...
                1: const pw.FractionColumnWidth(0.12), // Bill No
                2: const pw.FractionColumnWidth(0.20), // Product
                3: const pw.FractionColumnWidth(0.12), // Quantity
                4: const pw.FractionColumnWidth(0.12), // Pre Stock
                5: const pw.FractionColumnWidth(0.12), // New Stock
                6: const pw.FractionColumnWidth(0.20), // Remarks
              },
              children: [
                pw.TableRow(
                      ...
                      children: _columns.map((column) {
                        final alignment = (column == 'Quantity' ||
                            ...
                            : (column == 'Bill No'
                                ? pw.TextAlign.center
                                : pw.TextAlign.left);
                        // Show Date, Bill No, and Remarks only on the first transaction of the bill
      ...
      final file = File(
          '${outputDirectory?.path}/stock_in_report_${DateTime.now().millisecondsSinceEpoch}.pdf');
  ...
  Future<void> _generateExcel() async {
    ...
    try {
      ...
      var billNoStyle = excel.CellStyle(
        horizontalAlign: excel.HorizontalAlign.Center,
      );
      ...
      sheet.merge(
          ...
          excel.CellIndex.indexByColumnRow(columnIndex: 6, rowIndex: 0));
      sheet
          .cell(excel.CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 0))
          .value = excel.TextCellValue('Stock In Report');
      sheet
          .cell(excel.CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 0))
      ...
      for (int row = 0; row < _records.length; row++) {
        ...
        for (int col = 0; col < _columns.length; col++) {
          ...
          if (_columns[col] == 'Quantity' ||
              _columns[col] == 'Pre Stock' ||
              _columns[col] == 'New Stock') {
            cell.cellStyle = numberStyle;
          } else if (_columns[col] == 'Bill No') {
            cell.cellStyle = billNoStyle;
          } else {
      ...
      final fileName =
          'stock_in_report_${DateTime.now().millisecondsSinceEpoch}.xlsx';
