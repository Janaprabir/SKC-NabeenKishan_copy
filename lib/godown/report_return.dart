...
class ReportStockReturn extends StatefulWidget {
  const ReportStockReturn({super.key});

  @override
  _ReportStockReturnState createState() => _ReportStockReturnState();
}

class _ReportStockReturnState extends State<ReportStockReturn> {
  String? _empId;
  int? godownId;
  ...
  Future<void> _fetchData() async {
    ...
    const String baseUrl =
        'https://www.nabeenkishan.net.in/newproject/api/routes/StockReturnController/get_stock_return_report.php';

    String fromDateApi =
        DateFormat('yyyy-MM-dd').format(DateFormat('dd/MM/yyyy').parse(_fromDateController.text));
    String toDateApi =
        DateFormat('yyyy-MM-dd').format(DateFormat('dd/MM/yyyy').parse(_toDateController.text));
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> data = decodedResponse['entries'] ?? [];

        setState(() {
          _records.clear();
          godownName = data.isNotEmpty ? data[0]['godown_name'] : null;
          keeperName = data.isNotEmpty ? data[0]['keeper_name'] : null;

          // Group transactions by GRN No
          Map<String, Map<String, dynamic>> grnGroups = {};
          for (var returnEntry in data) {
            String formattedDate = '';
            if (returnEntry['stock_return_date']?.isNotEmpty ?? false) {
              try {
                DateTime date = DateTime.parse(returnEntry['stock_return_date']);
                formattedDate = DateFormat('dd/MM/yyyy').format(date);
              } catch (e) {
                formattedDate = returnEntry['stock_return_date'] ?? '';
              }
            }

            String grn = returnEntry['grn_no'] ?? '';
            if (!grnGroups.containsKey(grn)) {
              grnGroups[grn] = {
                'Employee Name':
                    '${returnEntry['emp_first_name'] ?? ''} ${returnEntry['emp_middle_name'] ?? ''} ${returnEntry['emp_last_name'] ?? ''}',
                'Date': formattedDate,
                'GRN No': grn,
                'Address': returnEntry['address'] ?? '',
                'DC Return': returnEntry['dc_retrun'] ?? '',
                'SRS No': returnEntry['srs_no'] ?? '',
                'Truck No': returnEntry['truck_no'] ?? '',
                'Trip No': returnEntry['trip_no'] ?? '',
                'Remarks': returnEntry['remarks'] ?? '',
                'Transactions': [],
              };
            }

            for (var transaction in returnEntry['transactions']) {
              grnGroups[grn]?['Transactions'].add({
          ...
          for (var grn in grnGroups.values) {
            for (var transaction in grn['Transactions']) {
              _records.add({
                'Employee Name': grn['Employee Name'],
                'Date': grn['Date'],
                'GRN No': grn['GRN No'],
                'Address': grn['Address'],
                'DC Return': grn['DC Return'],
                'SRS No': grn['SRS No'],
                'Truck No': grn['Truck No'],
                'Trip No': grn['Trip No'],
                'Remarks': grn['Remarks'],
                ...
                'IsGrnHeader': grn['Transactions'].indexOf(transaction) == 0,
              });
            }
          }
        });
  ...
  Future<void> _generatePdf() async {
    ...
    try {
        ...
        pw.MultiPage(
              ...
              children: [
                ...
                pw.Text(
                    ...
                    style: const pw.TextStyle(
                        fontSize: 14, color: PdfColors.white)),
                pw.Text(
                    'GRN No: ${grnNoController.text.isEmpty ? 'N/A' : grnNoController.text}',
                    style: const pw.TextStyle(
                        fontSize: 14, color: PdfColors.white)),
              ],
            ),
          ...
          build: (pw.Context context) => [
            ...
            pw.Table(
              ...
              columnWidths: {
                0: const pw.FractionColumnWidth(0.15), // Employee Name
                1: const pw.FractionColumnWidth(0.10), // Date
                2: const pw.FractionColumnWidth(0.10), // GRN No
                3: const pw.FractionColumnWidth(0.10), // Address
                4: const pw.FractionColumnWidth(0.08), // DC Return
                5: const pw.FractionColumnWidth(0.08), // SRS No
                6: const pw.FractionColumnWidth(0.08), // Truck No
                7: const pw.FractionColumnWidth(0.08), // Trip No
                8: const pw.FractionColumnWidth(0.10), // Remarks
                9: const pw.FractionColumnWidth(0.15), // Product name
                10: const pw.FractionColumnWidth(0.08), // Quantity
                11: const pw.FractionColumnWidth(0.08), // Pre Stock Level
                12: const pw.FractionColumnWidth(0.08), // New Stock Level
              },
              children: [
                pw.TableRow(
                      ...
                      children: _columns.map((column) {
                        final alignment = (column == 'Quantity' ||
                            ...
                            : (column == 'GRN No' ||
                                ...
                                ? pw.TextAlign.center
                                : pw.TextAlign.left;
                        // Show dispatch-level fields only on the first transaction of the GRN
                        final text = (column == 'Employee Name' ||
                                column == 'Date' ||
                                column == 'GRN No' ||
                                column == 'Address' ||
                                column == 'DC Return' ||
      ...
      final file = File(
          '${outputDirectory?.path}/stock_return_report_${DateTime.now().millisecondsSinceEpoch}.pdf');
  ...
  Future<void> _generateExcel() async {
    ...
    try {
      ...
      sheet.merge(
          ...
          excel.CellIndex.indexByColumnRow(columnIndex: 12, rowIndex: 0));
      sheet
          .cell(excel.CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 0))
          .value = excel.TextCellValue('Stock Return Report');
      sheet
          .cell(excel.CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 0))
      ...
      sheet
          ...
          .value = excel.TextCellValue('Keeper: ${godownName ?? 'N/A'}');
      sheet
      ...
      for (int i = 0; i < _columns.length; i++) {
        var cell = sheet.cell(
            excel.CellIndex.indexByColumnRow(columnIndex: i, rowIndex: 7));
      ...
      for (int row = 0; row < _records.length; row++) {
        ...
        for (int col = 0; col < _columns.length; col++) {
          var cell = sheet.cell(excel.CellIndex.indexByColumnRow(
              columnIndex: col, rowIndex: row + 8));
          // Show dispatch-level fields only on the first transaction of the GRN
          final value = (_columns[col] == 'Employee Name' ||
                  _columns[col] == 'Date' ||
                  _columns[col] == 'GRN No' ||
                  _columns[col] == 'Address' ||
                  _columns[col] == 'DC Return' ||
                  _columns[col] == 'SRS No' ||
                  _columns[col] == 'Truck No' ||
                  _columns[col] == 'Trip No' ||
      ...
      final fileName =
          'stock_return_report_${DateTime.now().millisecondsSinceEpoch}.xlsx';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ...
          : Center(
              child: Container(
                constraints: const BoxConstraints(maxWidth: 1200), // Set max width for the card
                child: Padding(
                          ...
                          if (_records.isNotEmpty) ...[
                            ...
                            Wrap(
                              ...
                              children: [
                                ...
                                _buildInfoChip('GRN No',
                                    grnNoController.text.isEmpty ? 'N/A' : grnNoController.text),
                              ],
                            ),
                          ],
                          const SizedBox(height: 24),
                                              ...
                                              return DataCell(
                                                Text(
                                                  ...
                                                  style: TextStyle(
                                                    ...
                                                    fontSize: 11,
                                                    fontWeight: (column == 'Employee Name' ||
                                                            column == 'Date' ||
                                                            column == 'GRN No' ||
                                                            column == 'Address' ||
                                                            column == 'DC Return' ||
